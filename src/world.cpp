...
void World::generate(int mapX, int mapY, int arrayX, int arrayY) {
	...
	for (int a = mapX*mapSize; a < mapX*mapSize+mapSize; a++) {
		for (int b = mapY*mapSize; b < mapY*mapSize+mapSize; b++) {
			...
			if (rand() % 10 == 0) {
				objects[arrayX][arrayY].push_back(new Object(window, 1, 1, a, b, "rock", &entitymap));
			}
		}
	}
...
void World::save(int mapX, int mapY, int arrayX, int arrayY) {
	...
	for (auto object : objects[arrayX][arrayY]) {
		file << object->getType() << " " << object->getWidth() << " " << object->getHeight() << " " << object->getX() << " "  << object->getY() << std::endl;
	}
	file.close();
}
...
void World::load(int mapX, int mapY, int arrayX, int arrayY) {
	...
	while (true) {
		...
		file >> tilex;
		file >> tiley;
		tilemaps[arrayX][arrayY][tilex][tiley] = new Tile(window, tilex, tiley, Tile::tileSize, Tile::tileSize, tiletype);
	}
	...
	while (!file.eof()) {
		...
		file >> objx;
		file >> objy;
		objects[arrayX][arrayY].push_back(new Object(window, objwidth, objheight, objx, objy, objtype, &entitymap));
	} 
	file.close();
}
